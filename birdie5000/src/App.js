...
class App extends Component {
	...
	deleteObservation = async observation => {
		...
		if (result) {
			...
			this.setState({
				observations: this.state.observations.filter(obs => obs.id.toString() !== observation.id),
				...
				<Router>
					<div className="container">
						...
						<Route
							...
							render={({ history }) => <ObsForm history={history} addObservation={this.addObservation} />}
						/>
					</div>
				</Router>
			</div>
